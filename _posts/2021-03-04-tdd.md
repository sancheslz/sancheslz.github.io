...
É por isso que o TDD, segue a filosofia do _fail fast_. Em um projeto guiado por testes, cada elemento foi devidamente testado, bem como suas relações, de forma que refatorar ou implementar algo apresentará, de imediato, se algo foi quebrado ou se continua funcionando.
...
Nesse exemplo, criamos uma calculadora simples que recebe um valor inicial ao instanciar o objeto e que pode receber novos valores a serem somados. Ao somar um novo valor, o mesmo será devolvido como resposta:
...
Nosso problema foi resolvido e nem precisamos "fazer testes" para isso. Calma jovem! O método `__add__` do Python permite mais do que fazer somas. Ele também permite que listas sejam concatenadas. Agora imagine alguém sabendo dessa própriedade da linguagem e que acredite que o cálculo feito aqui é com a soma de listas. Qual seria o resultado?
...
Segundo o autor, o Bode além de ser o mascote não oficial dos testes na comunidade Python, é também um grande mestra na cultura dos testes. Os caprinos podem alcançar lugares inimagináveis em solos perigosos e complexos, podendo, inclusive subir em árvores, acredite.
...
- _Arrange_ (organizar): nessa parte são definidas as condições pré-existentes para o nosso teste. Por exemplo, um teste de login, é necessário que haja um usuário, o mesmo seria criado aqui.
- _Act_ (agir): nessa parte é executada a ação principal do teste. Por exemplo, no teste de login, aqui seria o momento em que o usuário preenche seus dados e clica no botão de logar.
- _Assert_ (confirmar): nesta parte é verificado o resultado do teste. Por exemplo, no caso do login, podemos verificar seu o usuário está realmente logado e se foi redirecionado para a tela correta.
...
1. Usuário clica no botão de login e é redirecionado para o formulário
1. Usuário envia o formulário sem preencher os dados
1. Usuário preenche com dados incorretos (email)
1. Usuário preenche com dados incorretos (senha)
1. Usuário preenche com dados corretos
...
Arquivo: _test/test_calculator.py_
...
1. Criamos uma classe de testes e herdamos de `TestCase`
1. Criamos nosso primeiro teste iniciando com a palavrra `test_`
1. Dividimos nosso teste nas três parte do *AAA*:
   1. Instanciamos a calculadora com um valor incial `2`. Veja que é uma pré-condição que o cálculo seja realizado.
   1. Agimos sobre a instância da calculadora somando a ela outro valor.
   1. Confirmamos que o resultado da soma de `2` e `3` será `5`
...
Novamente temos um erro, isso quer dizer que regredimos em nosso teste, pois agora temos que `2 != 5`, o que de fato, é verdade. Vamos corrigir isso para fazer o teste passar.
...
Ok, mas será que alguém trabalha assim? Depende, há programadores que gostam de fazer teste com alterações mínimas e incrementais como fizemos aqui, há outros que preferem fazer o teste e escrever criar blocos de código.

Contudo, para quem está começando a usar o TDD, recomendo fortemente que faça os pequenos passos como fizemos aqui. Isso te ajudará a escrever testes melhores e organizar melhor o seu código.
