...
Além disso, há ocasiões em que o cenário depende de um maior detalhamento, para isso, pode-se utilizar as palavras `And` e `But`. Eles corresponderão à palavra-chave anterior. Por exemplo:

```gherkin
# ...
    Given I am logged
    When I click in logout
    And I wait for 5 seconds # => When I wait for 5 seconds
```

Em casos genéricos, ou em que haja uma enumeração de condições, pode-se utilizar a palavra-chage `*`:
...
Possui funcionamento similar ao `Example/Scenario`, com a diferença de que permite a execução de múltiplas vezes do cenários de acordo com as variações passadas em uma tabela de `Examples/Scenarios`:
...
As `data tables` permitem que um conjunto de dados seja passado para a respectiva etapa, bastando para isso construir uma tabela no formado Markdown:
